...
mutable struct CUSPARSESDDMMHandle{T} <: JLUST.AbstractKernelHandle
...
function JLUST.prepare(::CUSPARSEBackend, ::Type{<:Op{:SDDMM}},
    ...
    ws = _cusparse_workspace() do buf_sz, buf
        if buf === CUDA.CU_NULL
            cusparseSDDMM_bufferSize(handle(), transa, transb, α_ref, descA, descB, β_ref, descC, T, algo, buf_sz)
        else
            cusparseSDDMM_preprocess(handle(), transa, transb, α_ref, descA, descB, β_ref, descC, T, algo, buf)
        end
    end
...
function JLUST.execute(::CUSPARSEBackend, ::Op{:SDDMM, F_op},
                       u_A::USTensor{T}, u_B::USTensor, u_C::USTensor;
                       transa::Char='N', transb::Char='N',
                       alpha=one(T), beta=zero(T)) where {F_op, T<:_CUSPARSE_ELTYPES}
...
function JLUST.execute(h::CUSPARSESDDMMHandle{T},
                       u_A::USTensor, u_B::USTensor, u_C::USTensor;
                       alpha=one(T), beta=zero(T)) where T
    _cusparse_set_dense!(h.dnmat_A, nonzeros(u_A))
    _cusparse_set_dense!(h.dnmat_B, nonzeros(u_B))
    cusparseSpMatSetValues(h.spmat_C, nonzeros(u_C))
    cusparseSDDMM(handle(), h.transa, h.transb,
